...
// Supabase данните и настройките идват от config.js (зареден преди този файл)
const { url: SUPABASE_URL, anonKey: SUPABASE_ANON_KEY } = window.CLIENT_CONFIG.supabase;
const ENABLE_ORDERING = window.CLIENT_CONFIG.enableOrdering;
...
function renderCategoryButtons() {
    ...
    const activeClasses = "brand-bg text-white shadow-sm";
    ...
    container.innerHTML = visibleItems.map(item => `
        <div class="bg-white p-4 rounded-xl shadow-sm border border-gray-100 cursor-pointer relative" data-open-item="${item.id}">
            ...
            <div class="flex items-center justify-between mt-2">
                <p class="accent-text font-bold">${parseFloat(item.price).toFixed(2)} €</p>
                ${ENABLE_ORDERING ? `
                <button type="button" data-quick-add="${item.id}"
                    class="brand-bg text-white text-xs font-bold w-8 h-8 rounded-full cursor-pointer">+</button>
                ` : ''}
            </div>
        </div>
    `).join('');
...
function openItemModal(itemId) {
    ...
    if (addBtn) addBtn.classList.toggle("hidden", !ENABLE_ORDERING);
...
async function loadMenu() {
    ...
    if (cartFab) {
        if (!ENABLE_ORDERING) {
            cartFab.remove(); // без опция за поръчки - количката не съществува
        } else {
    ...
    try {
        ...
        if (bgData && bgData.value) {
            ...
            if (bgPhoto) {
                ...
                bgPhoto.classList.remove("brand-bg-gradient");
            }
